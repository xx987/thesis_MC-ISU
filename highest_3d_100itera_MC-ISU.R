...
for(l in 1:100){
  ...
  #hfunc <- function(lambda){(samples[,3] + 2.509275*samples[,2] + samples[,1]*xv)*(lambda/(lambda))}
  hfunc <- function(one,two,three){(three + 2.509275*two + one*xv)}
  ...
  numerator<-function(lambda){
    ...
    for (i in 1:M){
      ...
      nuo<-nuo+hfunc(samples[,1][i],samples[,2][i],samples[,3][i])*poster(lambda,alpha[i],beta_t[i], beta_o[i], sigm[i])/(prop_den[i]*prop_sig[i])
  ...
  denominator<-function(lambda){
    for (i in 1:M){
      deo<-deo+poster(lambda,alpha[i],beta_t[i], beta_o[i], sigm[i])/(prop_den[i]*prop_sig[i])
  ...
  weight<-c()
  for(i in 1:15){
    #res = sum(numerator(grid[i]))/sum(denominator(grid[i]))
    res = numerator(grid[i])/denominator(grid[i])
    ##this is the result of E(h(theta) | lambda, D)
    #res = numerator(grid[i])/denominator(grid[i])
  ...
  rtime_c <- append(rtime_c, time.taken)
  
  rinf_exp <- append(rinf_exp, range(mc_isu)[1])
  rsup_exp <- append(rsup_exp, range(mc_isu)[2])
  rmsdr <- append(rmsdr,mean(sdr))
  
  rinf_sdr <- append(rinf_sdr,range(sdr)[1])
  rsup_sdr<-append(rsup_sdr,range(sdr)[2])
  
  #rinf_the <-c(rinf_the, range(theo_v)[1])
  #rsup_the <- c(rsup_the, range(theo_v)[2])
}
rtime_c 
rinf_exp 
rsup_exp 
